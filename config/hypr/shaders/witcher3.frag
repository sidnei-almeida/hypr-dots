...
// ══════════════════════════════════════════════════════════════════════
//  The Witcher 3 — terroso e pictórico
// ══════════════════════════════════════════════════════════════════════
//
// O jogo nasceu com uma paleta lavada que a comunidade passou dez anos
// corrigindo, e o alvo declarado dessas correções sempre foi o mesmo: a
// demo de 2013, mais fria na sombra e mais dourada na luz do que a versão
// que saiu.
//
// É o perfil de croma mais alto do conjunto — saturação 1.02 e vibrance
// 0.24 — e o único que passa de 1.0 na saturação. Aqui o excesso é o ponto:
// Velen tem de pesar, Toussaint tem de doer de tão dourado. Mas o croma
// extra vem quase todo do VIBRANCE, não da saturação: o vibrance realça na
// razão inversa do que já é colorido, então o verde lavado de Velen sobe
// muito e a armadura vermelha do Geralt quase nada. Saturação plana no
// mesmo valor viraria mancha chapada — o ar de filtro.
//
// Ganho (1.055, 1.010, 0.895) é o dourado; lift com azul em 0.0028 contra
// vermelho em 0.0010 é o frio da sombra. Gama com verde em 1.008 é o
// pântano: um empurrão pequeno, e no MEIO-TOM, que é onde a folhagem vive.
// Na versão anterior isso só podia ser pedido tingindo a sombra, o que
// escurecia o pântano em vez de dar cor a ele.
//
// Clareza 0.17 pelo lado pictórico: o jogo tem muita textura de superfície
// — pedra, casca, tecido — que o contraste global não levanta.

// ── Exposição e curva ──
const float EXPOSICAO     = 1.010                     ;
const float CONTRASTE     = 1.340                     ;
const float PIVO          = 0.594                     ;
const float CALCANHAR     = 0.022                     ;
const float ABERTURA_PE   = 0.528                     ;
const float JOELHO        = 0.890                     ;

// ── Cor: lift/gama/ganho = sombras/meios/luzes ──
const vec3  LIFT          = vec3(0.0003, 0.0005, 0.0009);
const vec3  GAMA          = vec3(1.0000, 1.0100, 0.9820);  // verde no meio-tom: o pântano de Velen
const vec3  GANHO         = vec3(1.0550, 1.0100, 0.8950);  // o dourado de Toussaint

// ── Croma ──
const float SATURACAO     = 1.120                     ;  // o único do conjunto acima de 1.0
const float SAT_SOMBRA    = 0.950                     ;
const float VIBRANCE      = 0.340                     ;  // o croma extra vem daqui, não da saturação plana
const float BLEACH        = 0.000                     ;

// ── Ótica ──
const float CLAREZA       = 0.260                     ;
const float CLAREZA_RAIO  = 9.000                     ;
const float LIMITE_CLAREZA = 3.500                     ;
const float HALACAO       = 0.060                     ;
const float HAL_LIMIAR    = 0.740                     ;
const float HAL_RAIO      = 20.000                    ;
const vec3  HAL_TINT      = vec3(1.0000, 0.5600, 0.2600);
const float ABERRACAO     = 0.350                     ;
const float VINHETA       = 0.120                     ;
const float VIN_RAIO      = 0.350                     ;
const float VIN_CROMA     = 0.030                     ;

// ── Filme e painel ──
const float GRAO          = 0.018                     ;

...
// ══════════════════════════════════════════════════════════════════════
//  Lies of P — Krat, Belle Époque em ruínas
// ══════════════════════════════════════════════════════════════════════
//
// A direção de arte da Neowiz descreve Krat pela DUALIDADE do período:
// arquitetura Beaux-Arts e cartaz Art Nouveau de um lado, indústria e metal
// do outro. Em cor isso é uma coisa só — LAMPIÃO A GÁS contra PEDRA FRIA —
// e é o eixo inteiro deste perfil.
//
// Ganho (1.070, 1.000, 0.870) é o gás: o vão vermelho-azul de 0.20 é o
// maior do conjunto, empatado com o Alien. Lift com azul em 0.0038 contra
// vermelho em 0.0010 é a pedra. O meio-tom fica quase neutro de propósito
// (gama 1.000 / 0.996 / 1.004): em Krat a cor está nas FONTES de luz e no
// que elas não alcançam, nunca no ar entre as duas.
//
// HALAÇÃO em 0.11, com limiar 0.68, o mais baixo do conjunto — ou seja, ela
// pega mais cedo que as outras e espalha mais. Motivo: as fontes de luz de
// Krat são chama nua atrás de vidro, e chama atrás de vidro é o caso em que
// o halo é mais forte no mundo real. Sem isso o lampião vira um adesivo
// laranja colado na parede, que é como o jogo aparece na tela quando nada
// espalha.
//
// Souls-like passa metade do tempo lendo o cenário para achar caminho e o
// resto lendo a animação do inimigo no escuro, então aqui perder sombra não
// é perder clima, é perder a partida. O que garante a leitura é o pivô e a
// compensação de gama da tela, NÃO piso levantado — piso levantado põe véu
// na tela inteira e ainda assim não devolve detalhe nenhum.
//
// Saturação das sombras em 0.86 efetivo (0.98 x 0.88): a pedra de Krat é
// cinza, e sombra cinza é o que faz a chama parecer quente.

// ── Exposição e curva ──
const float EXPOSICAO     = 0.980                     ;
const float CONTRASTE     = 1.360                     ;
const float PIVO          = 0.588                     ;
const float CALCANHAR     = 0.022                     ;
const float ABERTURA_PE   = 0.565                     ;
const float JOELHO        = 0.870                     ;

// ── Cor: lift/gama/ganho = sombras/meios/luzes ──
const vec3  LIFT          = vec3(0.0003, 0.0005, 0.0009);  // a pedra fria de Krat
const vec3  GAMA          = vec3(1.0000, 0.9940, 1.0000);  // meio-tom neutro: em Krat a cor está nas fontes de luz
const vec3  GANHO         = vec3(1.0700, 1.0000, 0.8700);  // lampião a gás

// ── Croma ──
const float SATURACAO     = 1.040                     ;
const float SAT_SOMBRA    = 0.930                     ;
const float VIBRANCE      = 0.330                     ;
const float BLEACH        = 0.000                     ;

// ── Ótica ──
const float CLAREZA       = 0.240                     ;
const float CLAREZA_RAIO  = 9.000                     ;
const float LIMITE_CLAREZA = 3.500                     ;
const float HALACAO       = 0.110                     ;  // a mais forte: chama nua atrás de vidro
const float HAL_LIMIAR    = 0.680                     ;
const float HAL_RAIO      = 22.000                    ;
const vec3  HAL_TINT      = vec3(1.0000, 0.4800, 0.2000);
const float ABERRACAO     = 0.450                     ;
const float VINHETA       = 0.150                     ;
const float VIN_RAIO      = 0.320                     ;
const float VIN_CROMA     = 0.040                     ;

// ── Filme e painel ──
const float GRAO          = 0.020                     ;
const float PISO          = 0.000                     ;  // zero: o que salva a sombra é o pivô, não véu

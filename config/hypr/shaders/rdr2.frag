...
// ══════════════════════════════════════════════════════════════════════
//  Red Dead Redemption 2 — faroeste empoeirado
// ══════════════════════════════════════════════════════════════════════
//
// Laranja e petróleo, a dupla do cinema de faroeste desde sempre: pele e
// sol de um lado, sombra e céu do outro. O jogo já faz isso; o shader firma
// o que o painel VA entrega apagado.
//
// DUAS ESCOLHAS AQUI SÃO O CONTRÁRIO DAS DOS OUTROS PERFIS, e as duas têm o
// mesmo motivo — a bruma:
//
//   · CLAREZA em 0.09, a mais baixa do conjunto. O contraste local ataca
//     justamente a diferença entre o objeto e o ar em volta dele, que é
//     onde a neblina mora. Clareza alta limparia o horizonte, e RDR2 sem
//     bruma perde a distância: a montanha cola no cavalo.
//   · CONTRASTE global em 1.24, também o mais baixo. Fechar mais mataria a
//     mesma bruma pelo outro lado.
//
// Em troca, HALAÇÃO alta (0.075) e o RAIO mais largo do conjunto (24 px).
// Sol atravessando poeira é exatamente o caso em que o halo aparece — é o
// plano de RDR2 que todo mundo tira print. Aqui o halo faz o trabalho que o
// contraste não pode fazer: dá presença à luz sem endurecer a imagem.
//
// Grão 0.026, alto: a referência são os negativos Kodak de faroeste dos
// anos 70, a mesma que os presets de LUT da comunidade citam para este
// jogo.

// ── Exposição e curva ──
const float EXPOSICAO     = 1.010                     ;
const float CONTRASTE     = 1.260                     ;  // o mais baixo: fechar mais mata a bruma
const float PIVO          = 0.594                     ;
const float CALCANHAR     = 0.022                     ;
const float ABERTURA_PE   = 0.623                     ;
const float JOELHO        = 0.900                     ;

// ── Cor: lift/gama/ganho = sombras/meios/luzes ──
const vec3  LIFT          = vec3(0.0003, 0.0006, 0.0010);
const vec3  GAMA          = vec3(1.0060, 1.0000, 0.9880);
const vec3  GANHO         = vec3(1.0650, 1.0050, 0.8850);  // o sol; o petróleo sai pelo lift

// ── Croma ──
const float SATURACAO     = 1.080                     ;
const float SAT_SOMBRA    = 0.940                     ;
const float VIBRANCE      = 0.310                     ;
const float BLEACH        = 0.000                     ;

// ── Ótica ──
const float CLAREZA       = 0.140                     ;  // o mais baixo, pelo mesmo motivo do contraste
const float CLAREZA_RAIO  = 10.000                    ;
const float LIMITE_CLAREZA = 3.500                     ;
const float HALACAO       = 0.075                     ;
const float HAL_LIMIAR    = 0.720                     ;
const float HAL_RAIO      = 24.000                    ;  // o mais largo: sol atravessando poeira
const vec3  HAL_TINT      = vec3(1.0000, 0.5200, 0.2400);
const float ABERRACAO     = 0.400                     ;
const float VINHETA       = 0.110                     ;
const float VIN_RAIO      = 0.360                     ;
const float VIN_CROMA     = 0.020                     ;

// ── Filme e painel ──
const float GRAO          = 0.026                     ;

...
//
// Laranja e petróleo, que é a dupla do cinema de faroeste desde sempre:
// pele e sol de um lado, sombra e céu do outro. O jogo já faz isso; o
// shader só firma o que a tela VA entrega apagado.
//
// Contraste moderado (1.12) porque a poeira é parte do visual — fechar
// demais mata a bruma e o mundo perde a distância.
...
void main() {
    ...
    c = pretos(c, 0.10);
    c = contraste(c, 1.12, 0.47);
    c = tonalizar(c, vec3(0.91, 1.00, 1.09), vec3(1.11, 1.00, 0.87), 0.58);
    c = vibrance(c, 0.14);
    c *= vinheta(v_texcoord, 0.14, 0.31);

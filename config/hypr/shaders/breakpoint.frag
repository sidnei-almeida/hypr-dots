...
// ══════════════════════════════════════════════════════════════════════
//
// O QUE ESTAVA ERRADO NA VERSÃO ANTERIOR, e vale registrar porque o erro é
// comum: a sombra era tingida de (0.83, 1.08, 0.92). Isso é um vão de 0.25
// entre o vermelho e o verde, aplicado por um split-tone que vazava para o
// meio-tom inteiro. O resultado não era oliva — era uma folha de acetato
// verde na frente da tela.
//
// VERDE TÁTICO NÃO É VERDE FORTE, É VERDE ESCURO E SEM CROMA.
//
// Ranger green (#5B5F4A), o padrão de equipamento da OTAN desde os anos
// 2000, tem canais 0.357 / 0.373 / 0.290. Repare: o verde mal passa do
// vermelho — o vão é de 0.016, dezesseis vezes MENOR que o da versão
// anterior. Quem faz a cor ler como oliva é o AZUL, que cai 0.08 abaixo dos
// outros dois. Olive drab (#6B6B3A) segue a mesma lógica: vermelho e verde
// praticamente empatados, azul no chão.
//
// Então a receita aqui é o oposto da anterior:
//   · o vão verde-vermelho encolheu para 0.0008 no lift e 0.025 na gama;
//   · o AZUL é quem desce, tanto na sombra quanto no meio-tom;
//   · a saturação das SOMBRAS fica em 0.83 efetivo (0.96 x 0.86) — cor
//     tática é cor lavada, e material feito para não ser visto não tem
//     croma. Foi a única dessaturação que sobrou: a primeira versão
//     derrubava tudo para 0.63 e o resultado não era tático, era cinza.
//
// O resto do "tactical" não é cor, é ótica: clareza em 0.24, a mais alta do
// conjunto. É ela que dá a leitura de terreno — folhagem em camadas,
// silhueta destacada do fundo — que a mira do jogo precisa. Halação baixa e
// limiar alto (0.78) pelo mesmo motivo: só o sol vazando pela copa espalha,
// mais nada. Halo por toda parte em Auroa seria cenografia errada.
//
// As luzes puxam para o cinza FRIO (ganho 0.985 / 1.000 / 1.010). Duas
// razões: o céu encoberto de Auroa é frio, e o modo Cinema do monitor já
// esquenta a imagem — sem esse contrapeso o perfil chegaria amarelado na
// tela mesmo com os números certos no código.

// ── Exposição e curva ──
const float EXPOSICAO     = 0.990                     ;
const float CONTRASTE     = 1.320                     ;
const float PIVO          = 0.592                     ;
const float CALCANHAR     = 0.022                     ;
const float ABERTURA_PE   = 0.595                     ;
const float JOELHO        = 0.880                     ;

// ── Cor: lift/gama/ganho = sombras/meios/luzes ──
const vec3  LIFT          = vec3(0.0006, 0.0008, 0.0004);  // vão verde-vermelho de 0.0008: oliva, não verde
const vec3  GAMA          = vec3(0.9850, 1.0200, 0.9720);  // azul é quem desce no meio-tom
const vec3  GANHO         = vec3(0.9850, 1.0000, 1.0100);  // cinza frio; contrapeso ao modo Cinema

// ── Croma ──
const float SATURACAO     = 1.000                     ;
const float SAT_SOMBRA    = 0.900                     ;  // 0.96 x 0.86 = 0.83 efetivo na sombra
const float VIBRANCE      = 0.280                     ;
const float BLEACH        = 0.000                     ;

// ── Ótica ──
const float CLAREZA       = 0.320                     ;  // a mais alta do conjunto: leitura de terreno
const float CLAREZA_RAIO  = 8.000                     ;
const float LIMITE_CLAREZA = 4.500                     ;
const float HALACAO       = 0.035                     ;
const float HAL_LIMIAR    = 0.780                     ;  // só o sol pela copa espalha
const float HAL_RAIO      = 18.000                    ;
const vec3  HAL_TINT      = vec3(1.0000, 0.6600, 0.4200);
const float ABERRACAO     = 0.300                     ;
const float VINHETA       = 0.130                     ;
const float VIN_RAIO      = 0.340                     ;
const float VIN_CROMA     = 0.030                     ;

// ── Filme e painel ──
const float GRAO          = 0.016                     ;
const float PISO          = 0.000                     ;  // zero: preto absoluto continua absoluto

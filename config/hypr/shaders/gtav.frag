...
// GTA V — Los Santos, sol de meio-dia.
//
// O mais vivo dos seis: céu ciano, sol quente e um toque de bleach
// bypass, que é o que dá aquele estouro de cartaz publicitário — Los
// Santos é uma paródia da Califórnia, e a imagem tem de gritar um pouco.
//
// O bleach fica em 0.10 e não mais: acima disso a pele lava e todo mundo
// vira estátua de cera.
...
void main() {
    ...
    c = pretos(c, 0.12);
    c = contraste(c, 1.18, 0.48);
    c = tonalizar(c, vec3(0.91, 0.99, 1.11), vec3(1.09, 1.04, 0.90), 0.55);
    c = vibrance(c, 0.20);

    // Bleach bypass: mistura de volta um pouco da luminância pura, que é
    // o que a prata retida no negativo fazia — contraste sem cor.
    float l = dot(LUMA, c);
    c = mix(c, vec3(l), 0.10 * (1.0 - l));

    c *= vinheta(v_texcoord, 0.12, 0.32);
